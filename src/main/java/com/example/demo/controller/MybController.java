...
//		String oldstr = null;
	...
	private Map<String, Object> getHoliday(int year) {
		if (resMap != null && resMap.size() > 0 && (year+"").equals(String.valueOf(resMap.get("year")))) {
		...
		} else if (oldstr != null && year == 2022) {// 如果有本机缓存的话直接返回map
			System.out.println("---有本地字符串:2022年的---");
			resMap = new Gson().fromJson(oldstr, new TypeToken<Map<String, Object>>() {}.getType());
			...
			if (year > nowYear) {
				...
				try {
					 ...
					 resMap = new Gson().fromJson(rsa, new TypeToken<Map<String, Object>>() {}.getType());
					 resMap.put("year", year);
				} catch (IOException e) {
					e.printStackTrace();
				}
